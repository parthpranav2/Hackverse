...
import com.example.hackverse.GlobalClass.evtmGName
...
class TeamDetails : AppCompatActivity() {
    ...
    private fun handleTeammateEmailClose() {
        ...
        eventRef.child("evMaxTeamSize").addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val evSize = snapshot.getValue(String::class.java) // Assuming evSize is an integer
                if (evSize != null) {
                    findViewById<TextView>(R.id.txtTDmaxteamsize).text=evSize
                    Log.d("EventSize", "Event size is: $evSize")

                    // Check visibility condition
                    toggleButtonVisibility(evSize.toInt())
                } else {
    ...
    private fun toggleButtonVisibility(maxTeamSize: Int) {
    ...
    private fun fetchUserDetails(emailId: String) {
        ...
        userRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    val user = UserParsableModel(empName, empUName, null, null, null, empEmail,registeredEMP)
